...
blast_dbs=${4:-"nt nr swissprot pdbnt"}
...
cleanup_resources_on_error() {
    ...
    time yes | gcloud compute disks delete $name-pd --zone $zone
    exit 1;
}
...
time gcloud compute ssh $name-vm --zone $zone -- "sudo mkdir -p $mount_dir && sudo chmod a+rx $mount_dir && sudo mount -o discard,defaults /dev/${device} $mount_dir"
...
for db in $blast_dbs; do
    time gcloud compute ssh $name-vm --zone $zone -- docker run -v $mount_dir:/blast/blastdb:ro $blast_docker blastdbcmd -info -db $db
done
